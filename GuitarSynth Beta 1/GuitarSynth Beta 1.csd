...
rslider bounds(205, 273, 65, 65) channel("octave") range(-1, 1, 0, 1, 1) filmstrip ("knob.png", 128) popupPrefix("Octave Shift: "); OCTAVE
rslider bounds(288, 258, 90, 90) channel("mix") range(0, 1, 0.5, 1, 0.01)  filmstrip ("knob.png", 128) popupPrefix("Dry/Wet: "); DRY/WET
rslider bounds(395, 273, 65, 65) channel("port") range(0, 0.1, 0.01, 1, 0.001) filmstrip ("knob.png", 128) popupPrefix("Portamento Speed: "); PORT SPEED
...
signaldisplay bounds(198, 158, 135, 81) backgroundColour(255,255,255) colour("white") displayType("waveform") backgroundColour(0, 20, 0) zoom(-1) signalVariable("aPitch") channel("display") alpha(0.5); DISPLAY SCREEN SHOWING INPUT PITCH
signaldisplay bounds(336, 158, 135, 81) backgroundColour(255,255,255) colour("white") displayType("waveform") backgroundColour(0, 20, 0) zoom(-1) channel("display2") alpha(0.5); EMPTY DISPLAY SCREEN FOR UNDER NOTE NAME
label bounds(388, 184, 30, 30) channel("heading") text("-"), fontColour(255, 255, 255, 255) alpha(0.5); CHANGING NOTE NAME
...
rslider bounds(60, 255, 48, 145) range(0, 127, 0, 1, 1) channel("lforate") popupPrefix("LFO Rate: ") filmstrip("slider.png", 201); LFO RATE
rslider bounds(133, 255, 48, 145) range(0, 127, 0, 1, 1) channel("lfodely") popupPrefix("LFO Delay: ") filmstrip("slider.png", 201); LFO DELAY

rslider bounds(192, 255, 48, 145) range(0, 127, 0, 1, 1) channel("dcolfo") popupPrefix("DCO LFO: ") filmstrip("slider.png", 201); DCO LFO
rslider bounds(266, 255, 48, 145) range(0, 127, 0, 1, 1) channel("pwpwm") popupPrefix("Pulse Width Modulation: ") filmstrip("slider.png", 201); PULSE WIDTH MODULATION
rslider bounds(341, 255, 48, 145) range(0, 3, 0, 1, 1) channel("sublevl") popupPrefix("Sub Level: ") filmstrip("slider.png", 201); SUB OSCILLATOR LEVEL
rslider bounds(416, 255, 48, 145) range(0, 3, 0, 1, 1) channel("noislvl") popupPrefix("Noise Level: ") filmstrip("slider.png", 201); NOISE GENERATOR LEVEL

rslider bounds(483, 255, 48, 145) range(0, 3, 0, 1, 1), channel("hpffreq"), popupPrefix("High-Pass Frequency: ") filmstrip("slider.png", 201); HIGH-PASS FREQUENCY

combobox bounds(549, 232, 59, 20) text("Normal", "GT") channel("vcaenv"); VCA ENVELOPE TYPE
rslider bounds(554, 255, 48, 145) range(0, 127, 0, 1, 1) channel("vcaenvd") popupPrefix("VCA Envelope: ") filmstrip("slider.png", 201); VCA ENVELOPE LEVEL

rslider bounds(52.5, 452, 48, 145) range(0, 127, 127, 1, 1) channel("vcffreq") popupPrefix("VCF FRequency: ") filmstrip("slider.png", 201); VCF FREQUENCY
rslider bounds(116.5, 452, 48, 145) range(0, 127, 0, 1, 1) channel("vcfreso") popupPrefix("VCF Resonance: ") filmstrip("slider.png", 201); VCF RESONANCE
rslider bounds(179.5, 452, 48, 145) range(0, 127, 0, 1, 1) channel("vcfenvd") popupPrefix("VCF Envelope: ") filmstrip("slider.png", 201); VCF ENVELOPE
rslider bounds(242.5, 452, 48, 145) range(0, 127, 0, 1, 1) channel("vcflfo") popupPrefix("VCF LFO: ") filmstrip("slider.png", 201); VCF LFO

rslider bounds(554.75, 452, 48, 145) range(0, 127, 0, 1, 1) channel("crsrate") popupPrefix("Chorus Rate: ") filmstrip("slider.png", 201); CHORUS RATE

rslider bounds(296, 452, 48, 145) range(0, 127, 0, 1, 1) channel("envt1") popupText("0") filmstrip("slider.png", 201) alpha(0) active(0); ATTACK TIME
rslider bounds(361, 452, 48, 145) range(0, 127, 0, 1, 1) channel("envt2") popupText("0") filmstrip("slider.png", 201) alpha(0) active(0); DECAY TIME 1
rslider bounds(426, 452, 48, 145) range(0, 127, 0, 1, 1) channel("envt3") popupText("0") filmstrip("slider.png", 201) alpha(0) active(0); DECAY TIME 2
rslider bounds(491, 452, 48, 145) range(0, 127, 0, 1, 1) channel("envt4") popupText("0") filmstrip("slider.png", 201) alpha(0) active(0); RELEASE TIME
...
rslider bounds(1056, 92, 32, 32) range(0, 127, 0, 1, 1) channel("envl1") popupText("0") alpha(0) active(0); ATTACK LEVEL
rslider bounds(1056, 124, 32, 32) range(0, 127, 0, 1, 1) channel("envl2") popupText("0") alpha(0) active(0); DECAY MIDPOINT LEVEL
rslider bounds(56, 158, 32, 32) range(0, 127, 0, 1, 1) channel("envl3") popupText("0") alpha(0) active(0); SUSTAIN LEVEL
...
combobox bounds(352, 548, 51, 22) channel("pulse") text("00", "01", "02", "03"); PULSE OSCILLATOR TYPE
combobox bounds(403, 548, 51, 22) channel("sawtooth") text("00", "01", "02", "03", "04", "05"); SAWTOOTH OSCILLATOR TYPE
combobox bounds(454, 548, 58, 22) channel("sub") text("00", "01", "02", "03", "04", "05"); SUB OSCILLATOR TYPE 
}

combobox bounds(170, 0, 325, 45) channel("combobox") populate("*.snaps") channelType("string"); PRESET COMBOBOX
rslider bounds(50, 50, 48, 145) range(0, 10, 0, 1, 1) channel("PRESET") active(0) alpha(0), 201); HIDDEN SLIDER FOR REINITIALISING PRESETS
button bounds(0, 00, 170, 45) channel("Trigger") text("Basic Settings", "Advanced Settings") corners(5) value(1); TOGGLE FOR MAIN PAGE OR ADVANCED PAGE VIEW

filebutton bounds(495, 0, 175, 45) text("SAVE PRESET") populate("*.snaps", "test") mode("named preset"); SAVE NEW PRESET/ OVERWRITE PRESET BUTTON
...
instr 1; INPUT TRACKING AND PITCH DETECTION
    ...
    gkNoteName, gkNoteOffTrigger, gkPitch, gkOutL, gkOutR init 0; INITIALISE GLOBAL VARIABLES
    S_Notes[] fillarray "-", "C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "Bb", "B", "ERROR"; STORE POSSIBLE NOTE NAMES IN ARRAY
    
    S_NoteNameGUI sprintfk "text(%s)", S_Notes[gkNoteName]; GET NOTE NAME AT INDEX POSITION OF ROOT NOTE NUMBER
    cabbageSet metro(10), "heading", S_NoteNameGUI; SEND NOTE NAME TO USER INTERFACE

    giOctave[] fillarray 0.125, 0.25, 0.5, 1, 1.5, 2, 3; STORE OCTAVE MULTIPLIERS IN ARRAY
    kOctave chnget "octave"; GET VALUE OF OCTAVE KNOB
    ...
    gaInput = gaInput*((chnget:k("input")*2)); MULTIPLY INPUT AUDIO BY INPUT VOLUME KNOB
...
if gkAmp > 0.01 then; AVOID FRET NOISE BY ONLY TRACKING PITCH ABOVE AMPLITUDE THRESHOLD
   ...
   gkNoteName = 0; IF INPUT AMPLITUDE IS BELOW THRESHOLD, SET NOTENAME TO "-" IN USER INTERFACE
    ...
    kPort portk kfr, chnget:k("portspeed"); SMOOTH OUT PITCH CHANGES
if (kPort > 80 && kPort < 990) then; IF TRACKED PITCH IS WITHIN RANGE OF GUITAR NECK...
        gkPitch = kPort*giOctave[kOctave+3]; UPDATE GLOBAL PITCH VARIABLE
endif
    ...
    kPreset chnget "PRESET"; GET VALUE OF INVISIBLE PRESET SLIDER
    kPresetChanged changed kPreset; DETECT IF PRESET HAS CHANGED
    
if kPresetChanged == 1 then
    kTrig = 1
else
    kTrig = 0
endif
    
    kTrigChanged changed kTrig

if kTrig == 1 && kTrigChanged == 1 then; IF PRESET HAS CHANGED, RE-INIT SYNTH INSTRUMENT
    event"i", 2, 0, -1; TRIGGER SYNTH INSTRUMENT FOR INDEFINITE AMOUNT OF TIME
endif
    
if kPreset == 0 then; IF NO PRESET CHOSEN...
    outs gaInput, gaInput; OUTPUT DRY INPUT
endif
    
  
    ; WIDGETS SECTION ***********************
    if chnget:k("Trigger") == 1 then; CHANGE BETWEEN MAIN PAGE AND ADVANCED PAGE
    cabbageSet metro(10), "image1", "visible", 1
    cabbageSet metro(10), "image2", "visible", 0
    else
    cabbageSet metro(10), "image1", "visible", 0
    cabbageSet metro(10), "image2", "visible", 1
    endif
    
    cabbageSetValue "vu1", portk(gkAmp*5, .25), metro(10); UPDATE INPUT VU METRE VALUE
    
    cabbageSetValue "vu2", portk(gkOutL*5, .25), metro(10); UPDATE OUTPUT L VU METRE VALUE
    cabbageSetValue "vu3", portk(gkOutR*5, .25), metro(10); UPDATE OUTPUT R METRE VALUE
    ...
    cabbageSet metro(10), "knobglow1", S_InputGlow; UPDATE ALPHA (OPACITY) OF INPUT KNOB GLOW
    cabbageSet metro(10), "knobglow2", S_OutputGlow; UPDATE ALPHA (OPACITY) OF OUTPUT KNOB GLOW

    
    S_LED sprintfk "alpha(%f)", chnget:k("bypass"); CREATE STRING WITH ALPHA VALUE OF BYPASS BUTTON
    cabbageSet metro(10), "led", S_LED; UPDATE ALPHA (OPACITY) OF BYPASS LED (ALPHA IS 1 WHEN BYPASS IS ENABLED)
    
    S_Help1 sprintfk "alpha(%f)", chnget:k("Help1"); CREATE STRING WITH ALPHA VALUE OF HELP BUTTON
    cabbageSet metro(10), "HelpOverlay1", S_Help1; UPDATE ALPHA (OPACITY) OF HELP OVERLAY (ALPHA IS 1 WHEN HELP BUTTON IS ENABLED)
    
    S_VU sprintfk "alpha(%f)", 1-(chnget:k("Help1")); CREATE STRING WITH 1 - ALPHA VALUE OF HELP BUTTON
    cabbageSet metro(10), "vu1", S_VU; UPDATE ALPHA (OPACITY) OF INPUT VU METRE (ALPHA IS 0 WHEN HELP BUTTON IS ENABLED)
    cabbageSet metro(10), "vu2", S_VU; UPDATE ALPHA (OPACITY) OF OUTPUT L VU METRE (ALPHA IS 0 WHEN HELP BUTTON IS ENABLED)
    cabbageSet metro(10), "vu3", S_VU; UPDATE ALPHA (OPACITY) OF OUTPUT R VU METRE (ALPHA IS 0 WHEN HELP BUTTON IS ENABLED)
    
    kHelp2Changed changed chnget:k("Help2"); CHECK IF ADVANCED PAGE HELP BUTTON STATE HAS CHANGED
    kCounter init 0; INITALISE COUNTER AT 0 WHEN PLUGIN IS LOADED
    
if kHelp2Changed = 1 then; IF ADVANCED PAGE HELP BUTTON IS CLICKED...
    kCounter += 1; INCREMENT COUNTER
endif
    
if kCounter > 2 then; COUNTER RESETS AFTER 2 CLICKS
    kCounter = 0
endif
    
    ; MULTIPLE HELP OVERLAYS FOR ADVANCED PAGE USING COUNTER VALUE
if kCounter == 1 then; SHOW FIRST ADVANCED PAGE HELP FILE ON FIRST CLICK
    cabbageSet metro(10), "HelpOverlay2", "alpha(1)"
elseif kCounter = 2 then; SHOW SECOND ADVANCED PAGE HELP FILE ON SECOND CLICK
    cabbageSet metro(10), "HelpOverlay2", "alpha(0)"
    cabbageSet metro(10), "HelpOverlay3", "alpha(1)"
else; HIDE ALL ADVANCED HELP FILES ON THIRD CLICK
    cabbageSet metro(10), "HelpOverlay2", "alpha(0)"
    cabbageSet metro(10), "HelpOverlay3", "alpha(0)"
endif
...
;ENVELOPES
    kEnvL linsegr  0, gienvt1[iEnvT1], iEnvL1/127, gienvt1[iEnvT2], iEnvL2/127, gienvt3[iEnvT3], iEnvL3/127; ADSR VALUES (WITHOUT RELEASE VALUE - FIXED IN BETA 2)
    kEnvVCF linsegr  0, gienvt1[iEnvT1], iEnvL1, gienvt1[iEnvT2], iEnvL2 , gienvt3[iEnvT3], iEnvL3/127; ADSR VALUES (WITHOUT RELEASE VALUE - FIXED IN BETA 2)
...
; DCO BLOCK
    kNote           = gkPitch * (8  / (2^(kDcoRng + 1)));BASE NOTE CALCULATION : NOTE * DCORNG CORRECTION 
...
if (kDcoEnvd != 0) then; IF DCOENVD IS NOT 0...
    ...
    kNote = kNote * giOctave[chnget:k("octave")+2]; COMBINE NOTE WITH OCTAVE VALUE
...
if (kPulse == 0) then 
    ...
    ; WAVEFORM OPTIONS
    aOscTri vco2 gkAmp, kNote,0,0,0,0.2
    if (kPulse == 1) then
        aOsc1  vco2  p5, kNote,10,0,0,0.45; SQUARE 
    elseif (kPulse == 2) then 
        aOsc1   vco2  gkAmp, kNote,2,0.75
    elseif (kPulse == 3) then
        aOsc1   vco2  gkAmp, kNote ,2,  0.5 - (kLfoPw)*0.49/127  
        aOsc1 = aOsc1
    endif 
...
; WAVEFORM OPTIONS
if (kSub == 0) then 
    aOsc3   vco2,  gkAmp * kSubLevel, kNote/2,10

elseif (kSub == 1) then
    aOsc3   vco2  gkAmp * kSubLevel, kNote/2 ,2,0.75

elseif (kSub == 2) then
   aSub1 vco2  gkAmp* kSubLevel, kNote*2 ,10
   aSub2 vco2  gkAmp* kSubLevel, kNote/2 ,10
   aOsc3   =  (- aSub1 + 1) * (-aSub2 + 1) / 2 - 1

elseif (kSub == 3) then
   aSub1 vco2  gkAmp* kSubLevel, kNote*4 ,10
   aSub2 vco2  gkAmp* kSubLevel, kNote/2 ,10
   aOsc3   =  (- aSub1 + 1) * (-aSub2 + 1) / 2 - 1

elseif (kSub == 4) then
    aOsc3   vco2,  gkAmp * kSubLevel, kNote/4,10

elseif (kSub == 5) then
    aOsc3   vco2  gkAmp * kSubLevel, kNote/4 ,2,0.75
...
if (kNoisLvl == 0) then; BYPASS NOISE IF LEVEL IS SET TO 0
    ...
    aOsc4   noise gkAmp * kNoisLvl / 3, -0.9; GENERATE NOISE
...
; VCA VCF BLOCK (REMOVED OPTION IN BETA 3)
if (kVcaEnv == 1) then
    kEnv = kEnvL*2; NORMAL
...
; OUTPUT
if (kCrsRate != 0) then; ONLY RUN CHORUS CODE IF CHORUS SLIDER IS NOT ZERO I.E. CHORUS IS ON
    krate = gkCrsRate[kCrsRate]; GET MEASURED JUNO CHORUS RATE FROM CHORUS ARRAY
    aoffset = 0
    ...
    aoutChorusL,aoutChorusR 	StChorus	aoutPostFilter  ,aoutPostFilter  , gkCrsRate[kCrsRate], ktrem/2, aoffset, 0.5, 0.5; APPLY CHORUS UDO BY IAIN MCCURDY
else
    aoutChorusL = aoutPostFilter; BYPASS CHORUS OPCODE IF CHORUS SLIDER IS 0
    aoutChorusR = aoutPostFilter; BYPASS CHORUS OPCODE IF CHORUS SLIDER IS 0
endif 

    gklevel = chnget:k("output"); GET OUTPUT KNOB VALUE
    gkMix = (chnget:k("mix") * chnget:k("bypass")); GET MIX KNOB VALUE (AND MULTIPLY BY BYPASS BUTTON VALUE 0 OR 1 TO BYPASS EFFECT)
outs    (((aoutChorusL*kEnv)*gkMix)+((gaInput/2)*(1-gkMix)))*gklevel, (((aoutChorusR*kEnv)*gkMix)+((gaInput/2)*(1-gkMix)))*gklevel; OUTPUT SYNTH OUTPUT + DRY INPUT AT MIX RATIO, MULTIPLIED BY OUTPUT KNOB VALUE

    gkOutL rms gaInput*gklevel; STORE LEFT OUTPUT AMPLITUDE IN GLOBAL VARIABLE TO BE REFERENCED BY OUTPUT VU METRE
    gkOutR rms gaInput*gklevel; STORE RIGHT OUTPUT AMPLITUDE IN GLOBAL VARIABLE TO BE REFERENCED BY OUTPUT VU METRE
...
<CsScore>
f0 z
i1 0 z; RUN INSTRUMENT FOR APPROXIMATELY 7000 YEARS
</CsScore>
