...
signaldisplay bounds(198, 158, 135, 81) backgroundColour(0, 10, 0) colour("white") displayType("waveform") colour(240, 228, 66) zoom(-1) signalVariable("aPitch") channel("display") alpha(1); DISPLAY SCREEN SHOWING INPUT PITCH
signaldisplay bounds(336, 158, 135, 81) backgroundColour(0, 10, 0) colour("white") displayType("waveform") zoom(-1) channel("display2") alpha(1); EMPTY DISPLAY SCREEN FOR UNDER NOTE NAME
label bounds(352, 184, 100, 30) channel("heading") text("-"), fontColour(240, 228, 66) alpha(1); CHANGING NOTE NAME
...
rslider bounds(339, 535, 55, 55) range(0, 127, 0, 1, 1) channel("envt1") popupPrefix("Attack Time: ") filmstrip ("revknob.png", 20); ATTACK TIME
rslider bounds(367, 535, 55, 55) range(0, 127, 0, 1, 1) channel("envt2") popupPrefix("Decay Time: ") filmstrip ("revknob.png", 20); DECAY TIME
rslider bounds(394, 535, 55, 55) range(0, 127, 0, 1, 1) channel("envt3") popupPrefix("Sustain Level: ") filmstrip ("revknob.png", 20); SUSTAIN LEVEL (FIXED IN BETA 3)
rslider bounds(452, 535, 55, 55) range(0, 127, 0, 1, 1) channel("envt4") popupPrefix("Release Time: ") filmstrip ("revknob.png", 20); RELEASE TIME

rslider bounds(505, 430, 55, 55) range(0, 127, 0, 1, 1) channel("envl1") filmstrip ("revknob.png", 20); ATTACK LEVEL
rslider bounds(505, 460, 55, 55) range(0, 127, 0, 1, 1) channel("envl2") filmstrip ("revknob.png", 20); DECAY MIDPOINT LEVEL (FIXED IN BETA 3)
rslider bounds(505, 490, 55, 55) range(0, 127, 0, 1, 1) channel("envl3") filmstrip ("revknob.png", 20); SUSTAIN LEVEL
...
rslider bounds(50, 50, 48, 145) range(0, 10, 0, 1, 1) channel("PRESET") active(0) alpha(0) visible(0); HIDDEN SLIDER FOR REINITIALISING PRESETS
button bounds(0, 0, 170, 45) channel("Trigger") text("BASIC SETTINGS", "ADVANCED SETTINGS") corners(5) presetIgnore(1) value(1); TOGGLE FOR MAIN PAGE OR ADVANCED PAGE VIEW
...
instr 1; INPUT TRACKING AND PITCH DETECTION
    ...
    gkOutL, gkOutR, gkAmp, gkPitch, gkLiveMode, gkNoteName, gkMix,gkEnv, kloudprev, kCounter1, kCounter2 init 0; INITIALISE K-RATE VARIABLES
...
; WIDGETS SECTION ***********************
    ...
    gkOutL rms aInput*gklevel; OUTPUT VU METRE WRONG, FIXED IN BETA 3
    gkOutR rms aInput*gklevel; OUTPUT VU METRE WRONG, FIXED IN BETA 3
    cabbageSetValue "vu1", portk(gkAmp*5, .25), kMetro; SET VALUE OF INPUT VU METRE
    cabbageSetValue "vu2", portk(gkOutL*5, .25), kMetro; SET VALUE OF OUTPUT L VU METRE
    cabbageSetValue "vu3", portk(gkOutR*5, .25),kMetro; SET VALUE OF OUTPUT R VU METRE
...
if kKnobSlider == 0 then; IF SET TO KNOB MODE, MAKE ALL KNOBS VISIBLE AND ALL SLIDERS INVISIBLE
    ...
    cabbageSetValue "octave1", gkOctaveChoice; LINK VALUES OF KNOBS CORRESPONDING AND SLIDERS
    cabbageSetValue "port1", kPort1
    cabbageSetValue "mix1", kMix
    ...
    cabbageSetValue "octave", gkOctaveChoice; LINK VALUES OF KNOBS CORRESPONDING AND SLIDERS
    cabbageSetValue "port", kPort1
    cabbageSetValue "mix ", kMix
...
if (ktestampdel) > .2 then; DETECTION THRESHOLD (CHANGED FROM STATIC TO USER CHOSEN IN BETA 3)
...
if gkLiveMode == 1 then; IF LIVEMODE IS OFF
    cabbageSet kLiveModeChanged, "port", "alpha(0.5) active (0) popupText(Only Enabled in Live Mode)"; DEACTIVATE PORT KNOB
    cabbageSet kLiveModeChanged, "port1", "alpha(0.5) active (0) popupText(Only Enabled in Live Mode)"; DEACTIVATE PORT SLIDER
    ...
    cabbageSet metro(10), "port", "alpha(1) active(1) popupText()"; ACTIVATE PORT KNOB
    cabbageSet metro(10), "port1", "alpha(1) active (1) popupText()"; ACTIVATE PORT SLIDER
    cabbageSet metro(10), "envt1", "alpha(0.25) active(0) popupText(Disabled in Live Mode)"; DEACTIVATE ATTACK KNOB
    cabbageSet metro(10), "envt2", "alpha(0.25) active(0) popupText(Disabled in Live Mode)"; DEACTIVATE DECAY1 KNOB
    cabbageSet metro(10), "envt3", "alpha(0.25) active(0) popupText(Disabled in Live Mode)"; DEACTIVATE DECAY2 KNOB
    cabbageSet metro(10), "envt4", "alpha(0.25) active(0) popupText(Disabled in Live Mode)"; DEACTIVATE RELEASE KNOB
...
if gkLiveMode == 1 then; IF LIVEMODE IS OFF, FULL ADSR
    kEnvL linsegr  0, gienvt1[iEnvT1], iEnvL1/127, gienvt1[iEnvT2-1], iEnvL2/127, gienvt3[iEnvT3], iEnvL3/127, gienvt4[iEnvT4],0; ADSR VALUES
    kEnvVCF linsegr  0, gienvt1[iEnvT1], iEnvL1, gienvt1[iEnvT2-1], iEnvL2 , gienvt3[iEnvT3], iEnvL3, gienvt4[iEnvT4],0; ADSR VALUES  
else; IF LIVEMODE IS ON, ADS WHEN FIRST INITIALISED, NO RELEASE
    kEnvL linsegr  0, gienvt1[iEnvT1], iEnvL1/127, gienvt1[iEnvT2-1], iEnvL2/127, gienvt3[iEnvT3], iEnvL3/127; ADS, NO RELEASE
    kEnvVCF linsegr  0, gienvt1[iEnvT1], iEnvL1, gienvt1[iEnvT2-1], iEnvL2 , gienvt3[iEnvT3], iEnvL3/127;ADS, NO RELEASE
...
if gkLiveMode == 1 then; IF LIVEMODE IS OFF, CONSTANT NOTE PITCH
    kNote           = (p4*(giOctave[gkOctaveChoice+2])) * (8  / (2^(kDcoRng + 1)));BASE NOTE CALCULATION : (NOTE * OCTAVE CHOICE) * DCORNG CORRECTION
else; IF LIVEMODE IS ON, UPDATE NOTE PITCH IN REALTIME
    kNote           = (gkPitch*(giOctave[gkOctaveChoice+2])) * (8  / (2^(kDcoRng + 1)));BASE NOTE CALCULATION : (NOTE * OCTAVE CHOICE) * DCORNG CORRECTION
endif
